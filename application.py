...
application.config['SECRET_KEY'] = "chave_do_wenderson_123"
...
CORS(application) 
#CORS permite acessar a rota pelo editar.swagger.io
...
def add_products():
    data = request.json
    if 'name' in data and data['name'] and 'price' in data and data['price']:
...
@application.route('/')
def initial():
    return 'W Api Ecommerce - Status: Ativa'
...
def update_product(product_id):
    ...
    db.session.commit()
    return jsonify({"message": "Product update success"})
...
def remove_cart(product_id):
    ...
    if cart_item != None:
        ...
        db.session.commit()
        return jsonify ({"mensage": "Item removed from the cart successfull"})
...
#somente usado para homolog, isso não é usado em produção
